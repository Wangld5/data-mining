...
if __name__ == "__main__":
    ...
    theta = np.zeros(n)
    result_theta, TrainLoss, TestLoss = GradientDescent(1500000, x_data, y_data, theta, 0.00015, m, test_x_data, test_y_data)
    print(result_theta)
    ## paint
    x = [x/100000 for x in range(1, 1500001) if x%100000 == 0]
    plt.plot(x, TrainLoss, label="trainLossMean")
    plt.plot(x, TestLoss, label="testLossMean")
    plt.legend()
    plt.show()
